...
void matvec_omp_parallel(int M, int N, REAL Y[], REAL A[][N], REAL B[],
                         int num_tasks);
void matvec_omp_parallel_for(int M, int N, REAL Y[], REAL A[][N], REAL B[],
                             int num_tasks);
...
int main(int argc, char *argv[]) {
  ...
  printf("====================================================================="
         "=================================\n");
  printf("\tMatrix Vector Multiplication: Y[N] = A[N][N] * B[N], N=%d, %d "
         "tasks for omp_parallel\n",
         N, num_tasks);
  printf("---------------------------------------------------------------------"
         "---------------------------------\n");
  printf(
  ...
  printf("matvec_base:\t\t%4f\t%4f \t\t%g\n", elapsed * 1.0e3,
         (2.0 * N * N) / (1.0e6 * elapsed), check(Y_base, Y_base, N));
  printf("matvec_omp_parallel:\t%4f\t%4f \t\t%g\n",
         elapsed_omp_parallel * 1.0e3,
         (2.0 * N * N) / (1.0e6 * elapsed_omp_parallel),
         check(Y_base, Y_omp_parallel, N));
  return 0;
}
...
void matvec_base_sub(int i_start, int Mt, int M, int N, REAL Y[], REAL A[][N],
                     REAL B[]) {
  int i, j;
