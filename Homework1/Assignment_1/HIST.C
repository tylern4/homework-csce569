...
zero_histogram(histogram, gray_levels) int gray_levels;
...
calculate_histogram(image, histogram, length, width) int length, width;
short **image;
unsigned long histogram[];
{
  long i, j;
...
smooth_histogram(histogram, gray_levels) int gray_levels;
unsigned long histogram[];
{
  int i;
  ...
  new_hist[gray_levels] =
      (histogram[gray_levels] + histogram[gray_levels - 1]) / 2;
...
perform_histogram_equalization(image, histogram, gray_levels, new_grays, length,
                               width) int gray_levels,
    new_grays;
long length, width;
short **image;
unsigned long histogram[];
...
hist_long_clear_buffer(string) char string[];
{
  int i;
  for (i = 0; i < 300; i++)
    string[i] = ' ';
