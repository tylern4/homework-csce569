...
filter_image(the_image, out_image, rows, cols, bits_per_pixel, filter, type,
             low_high) int type;
short filter[3][3], **the_image, **out_image;
char low_high[];
long rows, cols, bits_per_pixel;

{
  int a, b, d, i, j, k, length, max, sum, width;
...
median_filter(the_image, out_image, rows, cols, size) int size;
...
median_of(elements, count) int *count;
short elements[];
{
  short median;
...
low_pixel(the_image, out_image, rows, cols, size) int size;
short **the_image, **out_image;
long rows, cols;
...
high_pixel(the_image, out_image, rows, cols, size) int size;
short **the_image, **out_image;
long rows, cols;
{
  int a, b, count, i, j, k, length, sd2, sd2p1, ss, width;
...
fsort_elements(elements, count) int *count;
short elements[];
{
  int i, j;
  j = *count;
...
setup_filters(filter_type, low_high, filter) char low_high[];
int filter_type;
short filter[3][3];
{
  int i, j;
